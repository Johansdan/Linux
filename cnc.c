...
void *detect_afk(void *arg)
{
    ...
    while(1)
    {
        if(time(NULL) - accinfo[the_fd].lastcmd_time > 999999)
        {
            char fbuf[512];
        ...
        if(time(NULL) - accinfo[the_fd].time_logged > 999999)
        {
            char fbuf[512];
...
void logcmd(int the_fd) {
    ...
    char timestamp[128]; 
    current_time = time(NULL);
    accinfo[the_fd].lastcmd_time = current_time;
    local_time = localtime(&current_time);
    strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S", local_time);
    sprintf(command, "echo \"(%s) %s: \\\"%s\\\"\" >> chat.log", timestamp, accinfo[the_fd].username, accinfo[the_fd].lastcmd);
    system(command);
}
